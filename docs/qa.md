...
| A19 | ADR 0011 | Una `timeDimension` con `dateRange` y sin `granularity` sólo filtra por fecha: el rango sigue en la CTE y la fecha no sale ni en el SELECT ni en el GROUP BY ni en las filas; sin rango y sin granularidad sigue siendo 400 | snapshot `rango-sin-granularidad.sql`; tests `plan/run/sqlite/cache`; curl 10-09 con `demo-agente-empresa-a`: `{"measures":["attendance.attendance_rate"],"dimensions":["departments.name"],"timeDimensions":[{"dimension":"attendance.date","dateRange":["2025-06-01","2025-08-31"]}]}` → 200 con dos filas (Ingeniería 92,59 y Ventas 91,55) | OK (auto) |
...
| M3 | Catálogo público (`GET /analytics/catalog`): las descripciones sirven como contexto para un agente | OK (10-09): la demo del agente traduce las 12 preguntas preparadas sólo con la vista pública; cambiar la descripción de `avg_score` cambió lo que el agente escribe (agregó el segmento `completed` solo) y agregar `departments.count` resolvió "cuántos departamentos hay". Pendiente como evolución: publicar en el catálogo las reglas que hoy van en el prompt (rango opcional, `dateRange` cerrado, forma de `filters`) |
| M4 | Mensajes de error: las sugerencias dicen qué hacer, no solo qué falló | OK (10-09): 12 códigos provocados en vivo por HTTP y leídos uno a uno; el resto (`QUERY_TIMEOUT`, `INVALID_CONSUMER`, `MISSING_PARAM`, `SCHEMA_DRIFT`, `SOURCE_UNAVAILABLE`, `UNKNOWN_QUERY`, `INVALID_DEFINITION`) con test. Hallazgo corregido: `NO_JOIN_PATH` sólo describía; ahora sugiere las dos salidas (fa37dfe) |
| M5 | Cambio de comportamiento: re-registrar una entidad con otra definición ahora es INVALID_DEFINITION (se arma un catálogo nuevo) | OK (10-09): probado en vivo contra `createCatalog()`: la misma definición es idempotente; otra definición con el mismo nombre → `INVALID_DEFINITION` en `reviews.name` con la sugerencia de elegir otro nombre o un catálogo nuevo. Aceptado como comportamiento correcto: la versión del catálogo (caché, agente) no puede cambiar en silencio |
...
| 10-09 | Primer arranque: el `api` moría con `ECONNREFUSED` porque `pg_isready` por socket daba por sana la instancia temporal del init de Postgres | Clon limpio, `docker compose up -d --build` y `exec api` inmediato: "service api is not running"; log del `api` con el error y log de `db` con dos "ready to accept" | 336ace5 (sonda por TCP y `restart: on-failure`) |
| 10-09 | `CLAUDE.md` superaba el límite de 40k caracteres que carga Claude Code (75k): la guía de revisor podía quedar fuera | Sesión limpia de Claude Code (Sonnet) en el clon: aviso al iniciar | dc7c193 (historial a `docs/bitacora-de-fases.md`) |
| 10-09 | README de arranque confuso: dos bloques repetían `docker compose up` sin decir cuál elegir y `cp .env.example .env` sugería que bastaba con eso | Lectura del README desde cero por el autor, intentando seguirlo | 2951240, 8cc3495 (dos caminos: manual A/B/C y con Claude Code) |
| 10-09 | Camino 2 (con Claude Code, Sonnet, sesión limpia) OK: levantó, esperó a la base sana, corrió la demo, explicó el SQL del caso con las ADR correctas y reportó 217 tests. Único desvío: no ofreció bajar los servicios | Prompt del README tal cual, en el clon con `CLAUDE.md` recortado | d871e4a (guía: ofrecer el `down`) |
