...
| Medio | `fillMissing` densifica la serie y multiplica las filas contra el tope de la clase de consumidor: el `LIMIT` corta, y corta en silencio | El tope sigue vigente (el relleno pasa **antes** del `LIMIT`, no después: ninguna consulta puede devolver más filas de las que su clase permite) y la respuesta no puede crecer sin límite. Lo que falta es el aviso: hoy el consumidor sólo puede deducir el corte comparando `rows.length` con el `rowLimit` de su presupuesto. Medido sobre la empresa C (1,06 M de filas de asistencia): por día y departamento sobre un rango de dos años, la misma consulta pasa de 4.392 filas con 12 departamentos completos a las 5.000 del tope de `dashboard`, con **7 de 12** departamentos en la respuesta y la serie cortada a mitad. Evolución prevista: estimar `buckets × ejes` en el planificador y, por encima del tope, o advertir en `meta.warnings` —la costura ya existe, es la de la razón anulada— o rechazar con `INVALID_QUERY` sugiriendo subir la granularidad o acortar el rango. Cube resuelve lo mismo desactivando el relleno sobre 10.000 filas; advertir es preferible a desactivar en silencio, que es el fallo que este ADR vino a corregir | documento |
